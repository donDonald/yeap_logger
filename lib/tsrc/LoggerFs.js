...
describe('yeap_logger.LoggerFs', ()=>{
    ...
    describe('#Logger.open and Logger.close', ()=>{
        ...
        it('logger.close, close not yet open logger', ()=>{
            assert.equal(false, logger.isOpen());
            logger.close();
            assert.equal(false, logger.isOpen());
        });

        it('logger.open, 1st time', ()=>{
            assert.equal(false, logger.isOpen());
            logger.open();
            assert.equal(true, logger.isOpen());
        });

        it('logger.open, 2nd time', ()=>{
            logger.open();
            assert.equal(true, logger.isOpen());
        });

        it('logger.close, 1st time', (done)=>{
            logger.close((err)=>{
                assert(!err);
                assert.equal(false, logger.isOpen());
                done();
        ...
        it('logger.close, 3rd time, no cb', ()=>{
            logger.close();
            assert.equal(false, logger.isOpen());
        });
    });
    ...
    describe('#Logger.flush', ()=>{
        ...
        it('logger.flush, logger is not open', (done)=>{
            logger.flush(done);
        });
        ...
        it('logger.flush, logger is empty', (done)=>{
            logger.flush(done);
        });

        it('logger.flush, write some data and flush, ', (done)=>{
